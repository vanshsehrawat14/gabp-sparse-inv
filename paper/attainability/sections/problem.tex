...
symmetric positive-definite (SPD) $A$, the natural pattern is the pattern of
$A$ itself when $\Gr$ is a tree
...
same class of sparse factorization also supports the solve operations used for
routed fields $\inv{A}e_s$ and fixed-point backward passes; separate public function
calls may factorize independently.

\paragraph{Why it is global, exact, and structurally sparse.}
$\inv{A}$ is generally dense and can couple distant nodes:
$\Gblock{u}{v}$ is a global functional of the whole graph, not merely of a
local neighbourhood. Yet if $\Gr$ has low treewidth, a fill-reducing
elimination order keeps the elimination fronts small. More precisely, let
$U_v$ be the later neighbours of node $v$ in the
...
general: both are $\Theta(n)$ on trees, while nested dissection on a
two-dimensional grid has $F=\Theta(n\log n)$ and
$W=\Theta(n^{3/2})$ \citep{george1973nested}. The Takahashi recurrence
...
edge weights $w$, and
$\kappa=(\lambda_{\max}(L(w))+\varepsilon)/\varepsilon$. SPD Cholesky needs no
