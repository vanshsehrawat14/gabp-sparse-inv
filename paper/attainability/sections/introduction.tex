...
a fixed point. In these examples, either the forward computation or the
implicit backward contains a linear solve. For an $N\times N$ matrix, a
conventional dense factorization costs $\bigO(N^3)$. When the coupling is
sparse on a low-treewidth graph, the
\emph{selected} inverse, meaning only the blocks of $\inv{A}$ on the factor's
filled sparsity pattern, is computable in $\bigO(n)$ at fixed block size and
elimination width
and $\bigO(n^{1.5})$ on two-dimensional grids with nested dissection and the
Takahashi recurrence \citep{george1973nested,takahashi1973,erismantinney1975},
never forming the dense inverse.
Recent selected-inversion and differentiable-sparse-solver systems establish
that this is an active implementation area
\citep{maillou2025serinv,fattah2025stiles,wang2025schwarz,torchsla2026};
...
perform comparably when scored on the selected pattern against a 64-bit
floating-point (fp64) oracle. The question examined here is instead:
\begin{quote}
...
\citep{zhu2019sparse,dwyer1948symbolic}, so reverse mode costs a constant
multiple of the forward \citep{baur1983complexity}. Both are stated once, as
...
\paragraph{Scope.}
The present evidence comes from controlled central processing unit (CPU)
experiments: routing grids up to
...
to low-treewidth (one-dimensional and quasi-two-dimensional) problems;
general three-dimensional problems can have frontal growth that makes the
claimed direct regime impractical at scale. The benchmark-scale
