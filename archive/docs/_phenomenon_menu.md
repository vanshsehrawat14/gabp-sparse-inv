...
**Update (2026-06-29, Task A matched-fit):** the
[matched-fit adjudication](_matched_fit_taskA.md) finds the confound-free maze
cell (exact-trained vs B2-trained, both eval exact) unmatchable for all 20 DEV
seeds -- B2-operator training cannot attain exact-training's in-distribution fit
(~3000x worse) -- so the operator-training advantage reads as
attainability/optimization, and the as-trained gap shrinks rather than grows with
size. DEV/exploratory only.
...
**Update (2026-06-29, Task A matched-fit):** the
[matched-fit adjudication](_matched_fit_taskA.md) re-reads H3 as
attainability/optimization, not bias: the high-rho / low-K effect cells are
exactly where the Neumann arm fits 2-8x worse and is mostly unmatchable, while
the matchable near-exact (high-K) cells collapse to the floor. The clean SURVIVE
branch does not fire. DEV/exploratory only; confirmatory HOLDOUT still pending.
